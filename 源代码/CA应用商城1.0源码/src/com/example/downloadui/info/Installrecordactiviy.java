...
public class Installrecordactiviy extends Activity{
	...
	String time=db.getdownloadinfourl("record","time","id",id);
	...
	recordinfoitem.setOnClickListener(new Softwareitemonclicklistener(changeid(id),db.getdownloadinfourl("record","category","id",id),"d"));
	...
	public class Checkboxfunction implements OnClickListener{
		...
		public void onClick(View v) {
			...
			case R.id.makesuredelecttext:
				for(int i=recorditemlayout.getChildCount()-1;i>=0;i--){
					item=(LinearLayout)recorditemlayout.getChildAt(i);
	...
	private String changeid(String id){
    	return id.substring(id.indexOf("·Ö")+1,id.length());
    }
}
